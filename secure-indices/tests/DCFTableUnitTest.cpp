...
bool check_dcf_table_correctness(uint128_t left_x, uint128_t** truth, uint128_t* res1, uint128_t* res2, uint64_t size, uint64_t depth, bool use_modulus, uint128_t modulus, uint128_t alpha = 0){
...
int main(int argc, char** argv){
    cout<<BLUE<<"Running test on DCFTable..."<<RESET<<endl; 
    ...
    cout<<"Generating DCF keys for DCF table..."<<endl;    
    ...
    sv1->dcf_table_oracle = new DCFTableServer(to_string(ctr), depth, size, u_table, cores, malicious);
    sv2->dcf_table_oracle = new DCFTableServer(to_string(ctr), depth, size, u_table, cores, malicious);
    svkey *svk1 = new svkey();
    svkey *svk2 = new svkey();
    copy_keys_to_server(cl->dcf_table_oracle->key[2*ctr], svk1, svk2);
    // TODO: uncomment when greaterThan implemented in DCF_source
    //copy_keys_to_server(cl->dcf_table_oracle->key[2*ctr + 1], svk1, svk2);
    sv1->add_keys(DCF_Table, svk1);
    sv2->add_keys(DCF_Table, svk2);
    ...
    INIT_TIMER;
    START_TIMER;
    sv1->dcf_table_oracle->eval_dcf_table(res1, gout_bitsize, true, group_mod);
    sv2->dcf_table_oracle->eval_dcf_table(res2, gout_bitsize, true, group_mod);
    ...
    assert(check_dcf_table_correctness(query_left_x, u_table, res1, res2, size, depth, true, group_mod, alpha) == true && "DCF table is incorrect!");
