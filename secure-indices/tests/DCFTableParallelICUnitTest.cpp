...
int main(int argc, char** argv){
    ...
    for (int i = 0; i < numPredicates; i++) {
        cout << "Setup for predicate " << i << endl;
        ...
        copy_keys_to_server(cl->dcf_table_oracle->key[2*i], svk1_l, svk2_l);
        copy_keys_to_server(cl->dcf_table_oracle->key[2*i + 1], svk1_r, svk2_r);
    ...
    INIT_TIMER;
    START_TIMER;
    workers[0] = thread(&dorydb::DCFTableServer::parallel_ic_eval_dcf_table, sv1->dcf_table_oracle, res1, gout_bitsize, true, group_mod, true);
    workers[1] = thread(&dorydb::DCFTableServer::parallel_ic_eval_dcf_table, sv2->dcf_table_oracle, res2, gout_bitsize, true, group_mod, true);
    workers[0].join();
    workers[1].join();
    ...
    for (int i = 0; i < numPredicates; i++) {
        assert(check_dcf_table_correctness(query_left_x, query_right_x, u_table, res1[i], res2[i], size, depth, true, group_mod, alpha) == true && "DCF table is incorrect!");
    }
