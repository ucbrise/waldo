...
def mkdirRemote(username, host, directory, key=None):
    if not key:
        cmd = "ssh -o StrictHostKeyChecking=no " + username + "@" + host + " \'mkdir -p " + directory + " \'"
    else:
        cmd = "ssh -o StrictHostKeyChecking=no -i " + key + \
              " " + username + "@" + host + " \'mkdir -p " + directory + " \'"
    executeCommand(cmd)
...
def mkdirRemoteHosts(username, hosts, directory, key=None):
    print hosts
    for host in hosts:
        mkdirRemote(username, host, directory, key)
...
def gitRemoteSetOrigin(directory, origin, host, key):
    cmd = "ssh -o StrictHostKeyChecking=no -i " + key + " -A " + host + " 'cd " + directory + "; git remote set-url origin git@github.com:" + origin + "'"
    executeCommand(cmd)

def gitPullRemote(directory, host, key, remote="origin", branch="main"):
    cmd = "ssh -o StrictHostKeyChecking=no -i " + key + " -A " + host + " 'cd " + directory + "; git pull %s %s'" % (remote, branch)
    executeCommand(cmd)
...
# Downloads folder remote_dir from all hosts in to local_dir
def getDirectory(local_dir, username, hosts, remote_dir, key=None):
    print hosts
    for h in hosts:
        if not key:
            cmd = "scp -o StrictHostKeyChecking=no -r " + \
                  username + "@" + h + ":" + remote_dir + " " + local_dir
        else:
            cmd = "scp -o StrictHostKeyChecking=no -i " + key + \
                  " -r " + username + "@" + h + ":" + remote_dir + " " + local_dir
        executeCommand(cmd)
...
# Sends file to remote host
def sendFile(local_file, h, username, remote_dir, key=None):
    if not key:
        cmd = "scp -o StrictHostKeyChecking=no  " + \
              local_file + " " + username + "@" + h + ":" + remote_dir
    else:
        cmd = "scp -o StrictHostKeyChecking=no -i " + key + \
              " " + local_file + " " + username + "@" + h + ":" + remote_dir
    executeCommand(cmd)
...
# Sends file to list of remote hosts
def sendFileHosts(local_file, username, hosts, remote_dir, key=None):
    for h in hosts:
        if not key:
            cmd = "scp  -o StrictHostKeyChecking=no " + \
                  local_file + " " + username + "@" + h + ":" + remote_dir
